...
for line in election_file:
    ...
    percentage_string = line_list[2].strip()
...
for state in state_names:
    ...
    if state in states_list:
        ...
        color = cm.Blues(votes_percentage[state] / 100)
        poly = Polygon(seg, facecolor=color, edgecolor='black')
        ax.add_patch(poly)

# Create a ScalarMappable object associated with the Blues colormap and normalized to [0, 100]
sm = cm.ScalarMappable(cmap=cm.Blues, norm=Normalize(0, 100))
...
plt.title('DEM vote percentage for each state in 2020 election')
