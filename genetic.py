...
class World_Map:
    def __init__(self, colors, adjacency_list):
        ...
        self.fitness, self.graph_nx = self.__convert_to_nxgraph(
            self.colors, self.adjacency_list)
    ...
    def print_me(self, figure_number=-1, figure_title=''):
        ...
        colors_edges = [color_mapping[element[2]['illegal']]
                        for element in edge_list]
        # plt.figure(figure_number)
        # plt.title(figure_title)
        # nx.draw_networkx(self.graph_nx, with_labels=True,
        #                  node_color=colors_nodes, edge_color=colors_edges)
        # plt.draw()
...
def generate_random_graph(number_of_nodes, probability_of_edge):
    ...
    G = nx.fast_gnp_random_graph(
        number_of_nodes, probability_of_edge, seed=None, directed=False)
    edges = []
...
def parent_selection(input_population, number_of_pairs, method='FPS'):
    ...
    if method == 'FPS':  # Fitness proportional selection
        ...
        probabilities = np.array(
            [person.fitness / fitness_sum for person in input_population])

        I_x = np.random.choice(np.arange(0, input_n),
                               number_of_pairs, p=probabilities)
        I_y = np.random.choice(np.arange(0, input_n),
                               number_of_pairs, p=probabilities)
...
def population_update(input_population, output_population_size, generation_change_method='Elitism',
    ...
    if generation_change_method == 'Elitism':
        ...
        output_population += input_population[:int(
            input_population_size * percentage_to_keep)]

        list_of_parent_pairs = parent_selection(
            input_population, input_population_size // 2)

        pair_index = 0
        while len(output_population) < output_population_size:
            child_1, child_2 = genetic_operator(
                list_of_parent_pairs[pair_index], method=genetic_op)
...
def generate_random_initial_population(population_size, n_nodes, al):
    ...
    print('A random population of ' +
          str(population_size) + ' people was created')
